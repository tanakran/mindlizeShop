...
class Salelist extends Component {
  ...
  constructor(prop){
    ...
    this.state={
      ...
      tableList: mapDataToState(prop.sale_stock),
  ...
  componentWillReceiveProps(nextProps){
    this.setState({
      dataDetail:[],
      tableList: mapDataToState(nextProps.sale_stock),
    });
  }
  ...
  updateStatusToDB(id){
    ...
    this.props.updateSale({data:data,id:id})
  }
  ...
  deleteDB(id){
    ...
    this.props.deleteSale({data:data,id:id})
  }
      ...
      rowData=>{
        if(rowData.is_pay==false){
          return {
            icon: 'delete',
            tooltip: 'Delete',
            onClick: (event, rowData) => {if(window.confirm('ต้องการลบข้อมูลนี้? จำนวนคงเหลือจะไม่คืนคลังสินค้า')){this.deleteDB(rowData.id)};}
          }
        }
      },
    ];
...
const mapStateToProps = (state) =>{
  ...
  return{
    sale_stock:state.firestore.ordered.sale_stock,
    saleWithID:state.firestore.data.sale_stock,
