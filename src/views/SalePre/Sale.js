...
import { addSalePre,checkOldBilPre } from '../../store/actions/salePreAction';

import ProductPreCard from '../../components/Card/ProductPreCard';
...
class Sale extends Component {
  ...
  constructor(prop){
    super(prop);
    const product = this.mapDataToState(prop.product);
    this.state={
        ...initData,
        ...col_inti,
        product:product,
  ...
  componentWillReceiveProps(nextProps){
    const product = this.mapDataToState(nextProps.product);
  ...
  getProductData(data){
    ...
    if(!isEmpty(cartData)){
        ...
        if(isEmpty(result)){
          ...
          if(index!=-1){
            ...
            cartData[index].amount=(amount+1);
          }
        }
    }else{
        cartData.push({
    ...
    Notify("เพิ่ม "+data.pd_name+"("+data.pd_code+") ลงในตะกร้า")
    this.setState({
        cartData:cartData,
        BilData:{
          ...this.state.BilData,
  ...
  setNewCart = (data) =>{
    ...
    this.setState({
      ...
      BilData:{
        ...this.state.BilData,
        ...this.autoCalTranspotCost(this.state.BilData,data),
        ...this.calBil(this.state.BilData,data),
  ...
  autoCalTranspotCost(BilData,cart){
    if(!isEmpty(BilData.customerSelected.post_code) && !isEmpty(BilData.lot_id) && !isEmpty(cart)){
  ...
  calBil(bildata,cartdata){
    ...
    let cart_subtotal_arr = cartdata.map((value)=>{
      try {
        return (value.amount*value.pd_price)
      } catch (error) {
  ...
  checkoutCart = () =>{
    ...
    if(!isEmpty(BilData.customerSelected)){
      if(!isEmpty(BilData.lot_id)){
          ...
          if(BilData.bil_discount>=0){
            ...
            if(BilData.transpotationCost>=0){
              ...
              for (let i = 0; i < cartData.length; i++) {
                let total = (cartData[i].pd_price*cartData[i].amount);
                  if(total>0){
                    bil_detial.push({
                      id:cartData[i].id,
                      pd_cost:cartData[i].pd_cost,
                      pd_price:cartData[i].pd_price,
              ...
              const dataToDB={
                ...
                lot_id:BilData.lot_id,
                lot_name:BilData.lot_name,
                lot_close_date:BilData.lot_close_date,
        ...
        Notify("กรุณาเลือก Lot ก่อนทำรายการ","error")
      }
    }else{
  ...
  addDatabase(){
    this.props.addSalePre({data:this.state.dataToDB});
    this.clearData();
    this.toggleModalADD();
  }
    ...
    if(!isEmpty(product)){
        ...
        /*showData = product.map(function(item){
            return (<Col xs="3" md="3" className="mb-3">
                    <a onClick={()=>self.getProductData(item)}><ProductPreCard propData={item}/></a>
                </Col>);
        })*/
        ...
        for(let i = startItem;i<lastItem;i++){
          showData.push(<Col xs="3" md="3" className="mb-3">
              <a onClick={()=>self.getProductData(product[i])}><ProductPreCard propData={product[i]}/></a>
          </Col>)
        }
...
const mapDispatchToProps = (dispatch) => {
  return{
    addSalePre: (data) => dispatch(addSalePre(data)),
    checkOldBilPre: (data) => dispatch(checkOldBilPre(data)),
  }
}
//export default (Home)
...
const useStyles = makeStyles((theme) => ({
  ...
  const BilCard = (props) => {
    ...
    const handleSetLotData = (value) =>{
      let newBilData = BilData;
        if(isEmpty(value)){
          newBilData.lot_id="";
          newBilData.lot_name="";
        }else{
          newBilData.lot_id=value.id;
          newBilData.lot_name=value.lot_name;
          newBilData.lot_close_date=ShowDate(value.lot_close_date);
        }
            ...
            <CardBody>
                <Row>
                    ...
                    <Col xs="12">
                        ...
                        <Autocomplete
                        onChange={(event, value) => handleSetLotData(value)}
                        id="highlights-demo"
                        options={lot}
                        getOptionLabel={(option) => option.lot_name}
                        renderInput={(params) => (
                            <TextField {...params} name="textAuto" onKeyPress={handleKeyPress} label="เลือก Lot สั่งซื้อ" variant="outlined" margin="normal" fullWidth/>
                        )}
                        renderOption={(option, { inputValue }) => {
                            ...
                            <div>
                                {parts.map((part, index) => (
                                <span key={index} style={{ fontWeight: part.highlight ? 700 : 400 }}>
                                    {part.text+" ("+ShowDate(option.lot_close_date)+")"}
                                </span>
                                ))}
                            </div>
                            );
  ...
  const TableOfSale = (props) => {
    ...
    const tableData = () =>{
      let rs = cart.map((v)=>{
        let total = parseFloat(v.amount)*parseFloat(v.pd_price)
        return (
          <tr>
            ...
            <td><b>{total}</b></td>
            <td>
              <Button outline  size="sm" color="danger" onClick={()=>delCart(v.id)}>
